...
(function ($, window, document, undefined) {
    ...
    */
    $.fn.a11yDatepicker = function (options) {
        var datepickers = $(this);

        datepickers.each(function () {
            new Datepicker($(this), options, true);
        });
    };
...
function Datepicker($this, options, modal) {
    ...
    $('#' + inputTextId).attr('placeholder', this.settings.format.toUpperCase());

    var id = calendarId;
    var target = inputTextId;
    ...
    //trap tab action for accessibility
    this.$id.trap();
}
...
Datepicker.prototype.popGrid = function () {
    ...
    daysInThisMonth = Math.floor((nextMonth.getTime() - thisMonth.getTime()) / (1000 * 60 * 60 * 24));
    if (month === 2) {
        daysInThisMonth = 31;
    }

    calendarHtml = '<table class="calendar" align="center" summary="select a date">';
    calendarHtml += '<thead>';
    ...
    calendarNavTtml = '<div class="month-wrap">' +
         ...
         '</div>';
    calendarHtml = calendarNavTtml + calendarHtml;

    for (var wDay = 0; wDay < firstWeekDay; wDay++) {
        calendarHtml += ((wDay === 0) || (wDay === 6)) ? '<td class="weekend">&nbsp;</td>' : '<td>&nbsp;</td>';
    }

    var weekDay = firstWeekDay;
    mm = this.addlead0(nextMonth.getMonth());
    mm = (mm === 0) ? 12 : mm;
    ...
    for (dayCounter = 1; dayCounter <= daysInThisMonth; dayCounter++) {
        ...
        calendarHtml += '<td';
        if (day === dayCounter) {
            calendarHtml += ' class="current"';
        }

        if (this.isIE) {
            calendarHtml += '><a role="button" data-value="' + this.addlead0(dayCounter) + '" title="' + this.getFullDate(dayCounter, mm, year) + '" tabindex="0"><span class="a11y-hide-visually">' + this.getDay(dayCounter, mm, year) + ' ' + this.months[month] + '</span>  ' + dayCounter + '  <span class="a11y-hide-visually">,' + year + '</span></a></td>';
        } else {
            calendarHtml += '><a role="button" data-value="' + this.addlead0(dayCounter) + '" tabindex="0"><span class="a11y-hide-visually">' + this.months[month] + '</span>  ' + dayCounter + '  <span class="a11y-hide-visually">,' + year + '</span></a></td>';
        }
        weekDay++;
    }

    for (i = weekDay; i < 7; i++) {
        calendarHtml += ((i === 0) || (i === 6)) ? '<td class="weekend">&nbsp;</td>' : '<td>&nbsp;</td>';
    }
...
Datepicker.prototype.handleGridKeyDown = function (e) {
    ...
    switch (e.keyCode) {
        ...
        case this.keys.space: {
            ...
            if ($(e.target).hasClass('next-month') || $(e.target).hasClass('prev-month')) {
                return false;
            }
...
Datepicker.prototype.defineLanguage = function () {
    ...
    this.instructionObj = {
        en: {
            ...
            navigation: 'A date picker calendar is opened, use tab key or shift+tab key to navigate, use ALT+PageDown or ALT+PageUp to select next month or previous month. [month-year] is currently displayed.'
        },
        fr: {
            nextMonth: 'aller le mois prochain: ',
            prevMonth: 'aller à mois précédent: ',
            navigation: "Un calendrier du sélecteur de date est ouvert, la touche de tabulation de l'utilisation ou shift + touche tabulation pour naviguer, utilisez ALT + touche PageDown ou ALT + PageUp pour sélectionner le mois prochain ou le mois précédent. [month-year] est actuellement affiché."
        },
        es: {
            nextMonth: 'ir al siguiente mes: ',
            prevMonth: 'ir al mes anterior: ',
            navigation: 'Un calendario selector de fecha se abre, utilice la tecla de tabulación o shift + tab clave para navegar, uso ALT + AvPág o ALT + RePág para seleccionar el mes que viene o el mes anterior. [month-year] se muestra actualmente.'
        }
    };
...
Datepicker.prototype.getFullDate = function (d, m, y) {  // m = 1-12 not 0-11.
    ...
    return (this.daysofweek[date.getDay()] + ", " + this.months[date.getMonth()] + " " + date.getDate() + " " + date.getFullYear());
